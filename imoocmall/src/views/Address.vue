                ...
                <div class="checkout-addr">
                            ...
                            <ul>
                                <li v-for="(item,index) in addressListFilter" :key="index"
                                    ...
                                    <dl>
                                        ...
                                        <dd class="tel"><a :href="'tel:'+item.tel">{{item.tel}}</a></dd>
                                    </dl>
                    ...
                    <div class="next-btn-wrap">
                        <router-link class="btn btn--m btn--red"
                                     :to="{path:'/orderConfirm',query:{addressId:addressListFilter[checkIndex]&&addressListFilter[checkIndex].addressId}}">
                            下一步
                        </router-link>
                    </div>
                </div>
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        methods: {
            init() {
                this.$ajax.get('/users/addressList')
                    .then((res) => {
            ...
            setDefault(addressId) {
                this.$ajax.post('/users/setDefault', {addressId})
                    .then((res) => {
                        if (res.data.status === 0) {
                            this.addressList = res.data.result;
                        }
            ...
            delAddress() {
                this.$ajax.post('/users/delAddress', {addressId: this.addressId})
                    .then((res) => {
                        if (res.data.status === 0) {
